...
def plot_mc_dynamic_pet(pet_orig, pet_mc, sub_out_dir, file_prefix):
    ...
    for idx, img in enumerate(image.iter_img(pet_orig)):
        ...
        plot_epi(
            ...
            display_mode="ortho",
            title=f"Frame #{idx}",
            cut_coords=(x_orig, y_orig, z_orig),
            vmin=vmin_orig,
            vmax=vmax_orig,
            output_file=f"orig_{idx}.png",
        )
    ...
    imageio.mimsave(
        os.path.join(sub_out_dir, f"{file_prefix}_desc-without_motion_correction.gif"),
        orig_images,
        duration=1,
        loop=0,
    )
    ...
    for idx, img in enumerate(image.iter_img(pet_mc)):
        ...
        plot_epi(
            ...
            display_mode="ortho",
            title=f"Frame #{idx}",
            cut_coords=(x_mc, y_mc, z_mc),
            vmin=vmin_mc,
            vmax=vmax_mc,
            output_file=f"mc_{idx}.png",
        )
    ...
    # Write the images to a GIF file
    imageio.mimsave(
        os.path.join(sub_out_dir, f"{file_prefix}_desc-with_motion_correction.gif"),
        mc_images,
        duration=1,
        loop=0,
    )
